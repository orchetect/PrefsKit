...
public protocol PrefsStoragePListImportable where Self: PrefsStorage {
    ...
    func load(plist url: URL, by behavior: PrefsStorageUpdateStrategy) throws
    
    /// Load the contents of a plist file and replace storage or merge into existing storage.
    func load(plist data: Data, by behavior: PrefsStorageUpdateStrategy) throws
    
    /// Load the contents of a plist file and replace storage or merge into existing storage.
    func load(plist dictionary: NSDictionary, by behavior: PrefsStorageUpdateStrategy) throws
}
