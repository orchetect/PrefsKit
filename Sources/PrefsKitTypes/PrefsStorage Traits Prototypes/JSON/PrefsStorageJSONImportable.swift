...
public protocol PrefsStorageJSONImportable where Self: PrefsStorage {
    ...
    func load(json url: URL, by behavior: PrefsStorageUpdateStrategy) throws
    
    /// Load the contents of a JSON file and replace storage or merge into existing storage.
    func load(json data: Data, by behavior: PrefsStorageUpdateStrategy) throws
    
    /// Load the contents of a JSON file and replace storage or merge into existing storage.
    func load(json string: String, by behavior: PrefsStorageUpdateStrategy) throws
}
